...
export function loadSettings(): TempleSettings {
  try {
    ...
    if (saved.blessingMessage && (!saved.blessingMessage.includes('Swamy') || saved.blessingMessage.includes('wealth'))) {
    ...
    if (
      ...
      saved.whatsappTemplate?.includes('mc=8398')
    ) {
