  ...
  return (
    <svg viewBox="0 0 180 240" width="180" height="240">
      ...
      {[186, 164, 136, 110, 84, 62].map((y, i) => (
        <line key={i} x1={20 + i * 10} y1={y} x2={160 - i * 10} y2={y} stroke={gold} strokeWidth="1" opacity="0.6" />
      ))}
      {[40, 65, 90, 115, 140].map((x, i) => (
        <path key={i} d={`M${x-8} 198 L${x-8} 188 Q${x} 183 ${x+8} 188 L${x+8} 198`} fill={shadow} opacity="0.35" />
      ))}
      <text x="90" y="237" textAnchor="middle" fontSize="8" fontFamily="serif" fill={gold} letterSpacing="0.5">Sri Balagurunadheeswara</text>
    </svg>
  );
};
